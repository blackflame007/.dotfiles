...
update() {
  ...
  do
    ...
    case "${type}" in
      "'Issue'") COLOR=$GREEN; ICON=$GIT_ISSUE; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq -r .html_url)"
      ;;
      "'Discussion'") COLOR=$WHITE; ICON=$GIT_DISCUSSION; URL="https://www.github.com/notifications"
      ;;
      "'PullRequest'") COLOR=$MAGENTA; ICON=$GIT_PULL_REQUEST; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq -r .html_url)"
      ;;
      "'Commit'") COLOR=$WHITE; ICON=$GIT_COMMIT; URL="$(gh api "$(echo "${url}" | sed -e "s/^'//" -e "s/'$//")" | jq -r .html_url)"
      ;;
      "'RepositoryDependabotAlertsThread'") COLOR=$RED; ICON=$BELL_DOT; URL="https://github.com/$(echo "$NOTIFICATIONS" | jq -r '.[0].repository.full_name')/security/dependabot"
      ;;
      *) COLOR=$WHITE; ICON=$BELL; URL="https://github.com/notifications"
      ;;
    esac
    ...
    notification=(
      ...
      drawing=on
      click_script="open -a 'Google Chrome.app' $URL; sketchybar --set github.bell popup.drawing=off"
    )
